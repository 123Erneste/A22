...
jer_void startReader(jer_char*, jer_char*, jer_char, jer_intg, jer_intg);
...
jer_intg mainReader(jer_intg argc, jer_char** argv) {
	...
	jer_char* program = argv[0];
	jer_char* input = argv[2];
	jer_char mode = MODE_FIXED;
	jer_intg size = 0, increment = 0, wrongNumber = 0;
...
************************************************************
*/
jer_void startReader(jer_char* program, jer_char* input, jer_char mode, jer_intg size, jer_intg increment) {

	ReaderPointer bufferp;		/* pointer to Buffer structure */
	FILE* fileHandler;			/* input file handle */
	jer_intg loadSize = 0;		/* the size of the file loaded in the buffer */
	jer_char symbol;			/* symbol read from input file */
...
jer_void bErrorPrint(jer_char* fmt, ...) {
	...
	(jer_void)vfprintf(stderr, fmt, ap);
	va_end(ap);
